    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-lx-calendar"></i> 远程申请详情</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="form-box">
                ...
                <el-form ref="form" :model="form" label-width="120px">
                    ...
                    <el-form-item label="故障自主描述">
                        <el-input type="textarea" readonly="" rows="5" v-model="data.faultdescself"></el-input>
                    </el-form-item>
                </el-form>
            </div>
            <div class="panel ">
...
<script>
    export default {
        ...
        methods: {
            ...
            getInfo(){
                this.$loading()
                this.$axios.post('/remoteapply/get',{id:this.id}).then((res) => {
                    this.$loading().close()
                    if (res.status.haserror) {
                        this.$message.error(res.status.errorshowdesc)
                    } else {
                        ...
                        for (var i=0;i<res.data.data.length;i++){
                            ...
                            if(res.data.data[i].currentstatus=="applying"){
                                ...
                                this.transTime(res.data.data[i].remotetime,"yyyy-MM-dd")+"需要远程协助"
                            }else if(res.data.data[i].currentstatus=="comfirm"){
                                text="后台用户:"+res.data.data[i].username+"确认，指派"+
                                res.data.data[i].staffname+"远程协助"
