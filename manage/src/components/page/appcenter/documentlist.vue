    ...
    <div class="table">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-lx-calendar"></i> {{pagetitle}}</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
...
<template slot-scope="scope">
     {{transTime(scope.row.createtime)}}
                ...
                <template slot-scope="scope" v-if="scope.row.applystatus != 'unlock'">
                    <el-button type="text" @click="dealapply(scope.row.id,true)">
                        可看</el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            deal(id, bool) {
                this.$loading()
                this.$axios.post('/unlockapply/dealapply', {
                    id: id,
                    agree: bool
                }).then((res) => {
                    this.$loading().close()
